...
public class DatagramClientSample {
    ...
    public void sendDatagramData(String data) {
        try {
            DatagramSocket client = new DatagramSocket(); //1
            InetAddress address = InetAddress.getByName("127.0.0.1"); //2
            byte[] buffer = data.getBytes();
            DatagramPacket packet = new DatagramPacket(buffer, 0, buffer.length, address, 9999); //3
            client.send(packet); //4
